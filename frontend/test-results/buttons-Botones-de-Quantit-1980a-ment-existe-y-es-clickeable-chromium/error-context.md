...
- Name: buttons.spec.ts >> Botones de QuantitySelector >> botón decrement existe y es clickeable
- Location: tests-functional/buttons.spec.ts:130:7
...
Locator: getByRole('button', { name: /decrease/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /decrease/i }).first()
