...
- Name: buttons.spec.ts >> Botones de CartPage >> botón Remove existe y es clickeable
- Location: tests-functional/buttons.spec.ts:42:7
...
Locator: getByRole('button', { name: /remove/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /remove/i }).first()
